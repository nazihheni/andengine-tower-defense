...
public class Projectile extends Sprite{
        ...
        Enemy target;
        Tower source;
        public final static float speed = 55f; //movement speed higher is faster (distance to move per update)
        ...
        public Projectile(float pX, float pY, float pWidth, float pHeight,
                        TextureRegion pTextureRegion,VertexBufferObjectManager tvbom, Scene sc) {
        ...
        public void shoot(final ArrayList<Enemy> arrayEn, final BaseGameActivity myContext) {
        	...
        	dY = target.getMidY() - source.getMidY();//vertical distance from tower to enemy
        	//Enemy.speed;
        	//Projectile.speed;
        	...
        	double t2 = ( -(Math.sqrt(Math.pow(Projectile.speed,2) * Math.pow(d0, 2) - Math.pow(dY, 2) * Math.pow(Enemy.speed,2)) + 
        			Math.sqrt(Math.pow(d0, 2) - Math.pow(dY, 2)) * Enemy.speed) / 
        			( Math.pow(Projectile.speed,2) - Math.pow(Enemy.speed, 2) ) );
        	dX = Math.sqrt(Math.pow(Math.sqrt(t1 - Math.sqrt(Math.pow(d0, 2)-Math.pow(dY, 2))),2)+Math.pow(dY, 2));
        	
        	t1 = (Math.sqrt((Math.pow(Projectile.speed,2) * Math.pow(d0, 2)) - (Math.pow(dY, 2) * Math.pow(Enemy.speed,2))) - Math.sqrt(Math.pow(d0, 2) - (Math.pow(dY, 2)) * Enemy.speed)) 
					/ 
        			( Math.pow(Projectile.speed,2) - Math.pow(Enemy.speed, 2) ) ;
        	//dX = Math.sqrt(Math.pow(Math.sqrt(t1 - Math.sqrt(Math.pow(d0, 2)-Math.pow(dY, 2))),2)+Math.pow(dY, 2));
        	dX = Enemy.speed*t1 - Math.sqrt(
        			Math.pow(d0, 2)-Math.pow(dY, 2)
        		);
        	...
        	*/
        	...
        	double a = target.getXSpeed() * target.getXSpeed() + target.getYSpeed() * target.getYSpeed() - Projectile.speed * Projectile.speed;
	        ...
	        */
        	...
        	dY = target.getMidY() -  this.getMidY();
        	//dX = (target.getMidX() -  this.getMidX())/Math.abs(target.getMidX() -  this.getMidX())*dX;
